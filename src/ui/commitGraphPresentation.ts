...
export function createCommitGraphPresentation(state: RepositoryState): CommitGraphPresentation {
  ...
  try {
    ...
    const head = state.currentLocation.kind === "branch" && currentBranch ? { targetKind: "branch" as const, targetCommitId: state.currentLocation.head.id, x: currentBranch.x, y: currentBranch.bounds.top - 10, targetY: currentBranch.bounds.top - 2 } : state.currentLocation.kind === "detached" && nodeById.has(state.currentLocation.head.id) ? { targetKind: "commit" as const, targetCommitId: state.currentLocation.head.id, x: nodeById.get(state.currentLocation.head.id)!.x, y: 30, targetY: nodeById.get(state.currentLocation.head.id)!.y - 8 } : undefined;
...
function placeLocalBranches(state: RepositoryState, nodeById: ReadonlyMap<string, CommitGraphNode>, currentId: string | undefined): readonly GraphRef[] {
  ...
  const groups = [...grouped.entries()].sort(([leftId], [rightId]) => {
    const currentPriority = Number(rightId === currentId) - Number(leftId === currentId);
