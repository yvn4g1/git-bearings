...
function previewGraph(graph: CommitGraphPresentation, preview: CommandPreviewPresentation | undefined): CommitGraphPresentation {
  ...
  const facts = new Map(graph.nodes.map((node) => [node.commitId, node])); const predictions = new Map<string, { readonly x: number; readonly y: number }>(); const factRefRight = Math.max(0, ...graph.localBranches.map((ref) => ref.bounds.left + ref.bounds.width)); let nextX = factRefRight + 86;
  const predictionCommits = preview.map.predictions.map((item, index) => { const parents = item.parentCommitIds.map((id) => facts.get(id)).filter((item): item is NonNullable<typeof item> => item !== undefined); const basedOn = item.basedOn ? (item.basedOn.kind === "existing" ? facts.get(item.basedOn.id) : predictions.get(item.basedOn.id)) : undefined; const anchor = parents[0] ?? basedOn; const point = { x: Math.max(anchor ? anchor.x + 154 : graph.width - 120, nextX), y: anchor ? anchor.y + (item.basedOn ? 34 : 0) : 48 + index * 34 }; nextX = point.x + 154; predictions.set(item.id, point); return { id: item.id, label: "Prediction" as const, description: item.description, ...point, visualState: "related" as const }; });
  ...
  const predictionPointers = preview.map.pointers.flatMap((pointer, index) => { const target = pointer.target.kind === "existing" ? facts.get(pointer.target.id) : predictions.get(pointer.target.id); return target ? [{ label: pointer.label, x: target.x, y: target.y - (pointer.kind === "branch" ? 48 : 78) - index * 18, toX: target.x, toY: target.y - 9, kind: pointer.kind }] : []; });
  return { ...graph, predictionCommits, predictionEdges, predictionPointers, width: Math.max(graph.width, nextX + 40) };
}
