...
test("parser supports the specified grammar with typed semantics", () => {
  const cases: readonly [string, unknown][] = [
    ...
    ["git fetch", { kind: "fetch" }], ["git fetch origin", { kind: "fetch", remote: "origin" }], ["git push", { kind: "push", target: { kind: "default" }, setUpstream: false }], ["git push origin main", { kind: "push", target: { kind: "explicit", remote: "origin", branch: "main" }, setUpstream: false }], ["git push -u origin main", { kind: "push", target: { kind: "explicit", remote: "origin", branch: "main" }, setUpstream: true }], ["git pull", { kind: "pull", target: { kind: "default" }, rebase: false }], ["git pull origin main", { kind: "pull", target: { kind: "explicit", remote: "origin", branch: "main" }, rebase: false }], ["git pull --rebase", { kind: "pull", target: { kind: "default" }, rebase: true }], ["git pull --rebase origin main", { kind: "pull", target: { kind: "explicit", remote: "origin", branch: "main" }, rebase: true }], ["git merge main", { kind: "merge", branch: "main" }], ["git rebase main", { kind: "rebase", upstream: "main" }],
  ];
...
test("parser keeps option arguments, stash selectors, and force lease boundaries strict", () => {
  for (const input of ["git commit -m -m", "git stash push -m", "git stash push -m -u", "git stash push -m -m"]) assert.equal(parseGitCommand(input).kind, "parseFailure", input);
  for (const input of ["git commit -m --amend", "git stash push -m --unknown", "git stash list --foo", "git stash list -u"]) assert.equal(parseGitCommand(input).kind, "unsupportedOption", input);
  assert.equal(parseGitCommand("git stash list extra").kind, "parseFailure");
  for (const input of ["git stash apply stash@{0}", "git stash apply stash@{1}", "git stash apply stash@{12}"]) assert.equal(parseGitCommand(input).kind, "parsed", input);
  for (const input of ["git stash apply stash@{01}", "git stash apply stash@{00}", "git stash apply stash@{-1}", "git stash apply stash@{+1}", "git stash apply stash@{1.5}", "git stash apply stash@{x}", "git stash apply stash@{}", "git stash apply stash@{9007199254740992}"]) assert.equal(parseGitCommand(input).kind, "parseFailure", input);
  assert.equal(parseGitCommand("git push --force-with-lease=main origin main").kind, "highRisk");
});
