...
test("real Git stash keeps untracked by default, includes it with -u, and apply/pop have distinct shelf effects", async () => {
  ...
  try {
    ...
    await git(path, "stash", "apply"); assert.equal((await git(path, "stash", "list")).includes("tracked"), true); await git(path, "restore", "--staged", "a.txt"); await git(path, "restore", "a.txt"); await git(path, "stash", "pop"); assert.equal((await git(path, "stash", "list")).includes("tracked"), false);
...
test("real Git fetch and push alter remote refs only through local bare remote operations", async () => {
  ...
  try {
    ...
    await execFile("git", ["clone", "--branch", "main", bare, clone]); await git(clone, "config", "user.name", "Simulator Test"); await git(clone, "config", "user.email", "simulator@example.test"); await commit(source, "remote.txt", "remote\n", "remote"); const old = await git(clone, "rev-parse", "refs/remotes/origin/main"); await git(source, "push"); await git(clone, "fetch", "origin"); assert.notEqual(await git(clone, "rev-parse", "refs/remotes/origin/main"), old); await git(clone, "merge", "--ff-only", "origin/main");
    await commit(clone, "local.txt", "local\n", "local"); await writeFile(join(clone, "uncommitted.txt"), "not pushed\n"); await git(clone, "push", "-u", "origin", "main"); assert.equal(await git(bare, "show", "main:local.txt"), "local\n"); await assert.rejects(git(bare, "show", "main:uncommitted.txt"));
