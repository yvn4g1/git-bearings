...
- No external network: MVPでは外部通信しない。Gitのimplicit lazy fetchも許可せず、partial cloneは安全側に倒して対象外とする。
...
Webviewを導入するPhase 0からCSP、Repository文字列の安全な描画、message validationを適用する。Phase 7はこれらの最終hardeningと攻撃的testを行う段階である。MVP完了後のadversarial reviewでも、partial cloneのimplicit fetch、copy可能なGoal commandのshell expansion、巨大process output、Workspace Trustなど、通常利用では表面化しにくい境界を追加でhardeningする。
...
No external networkを守るため、partial cloneは現在対応対象外とする。Git CLIの出力が安全上限を超えた場合も、途中まで得られた情報をFactとして表示せず、その取得単位をunavailableとして扱う。Goal / Previewは学習支援のための表示であり実行機能ではないが、表示されたcommandがterminalへcopyされる可能性も安全境界に含める。
